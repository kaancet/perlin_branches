...
def bloom(j,i,degrow:bool=True):
...
def draw():
    global origins,current_lengths,isGrowing,draw_limit
    py5.background(py5.color(51, 5, 100))

    for j,b in enumerate(branches):
        for i,p in enumerate(b.branch_points[::-1]):
            ...
            if p[1]>draw_limit:
                py5.line(p[0]-current_lengths[j][i],p[1],p[0]+current_lengths[j][i],p[1])
                bloom(j,i,degrow=False)
                
            if py5.frame_count%10==0 and draw_limit>=0:
                draw_limit-=0.1
